...
const USAState = (props) => {
  ...
  return (
    <g ref={stateRef} id={props.state} transform={`translate(${groupOffset})`} width={width} height={height}>
      ...
      <text textAnchor="middle" className="state" style={{pointerEvents: "none"}} x={textX} y={textY}>{props.state}</text>
      <text textAnchor="middle" className="state-name" s style={{pointerEvents: "none"}} x={textX} y={textY}>{props.stateName}</text>
    </g>
  );
}
